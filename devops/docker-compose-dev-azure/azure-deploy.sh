...
resourcename="boatyard"  # lower case, only
resourcegroup="boatyard_rg"
...
# git clone https://github.com/apilogicserver/boatyard.git
# cd boatyard
...
az group create --name boatyard_rg --location "westus"
az appservice plan create --name myAppServicePlan --resource-group boatyard_rg --sku S1 --is-linux
az container create --resource-group boatyard_rg --name boatyard --image apilogicserver/boatyard:latest --dns-name-label boatyard --ports 5656 --environment-variables 'VERBOSE'='True'  'APILOGICPROJECT_CLIENT_URI'='//boatyard.westus.azurecontainer.io:5656'
